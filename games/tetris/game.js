// slicegames/games/tetris/game.js
(() => {
'use strict';
...
boardEl.width  = cell * COLS;
boardEl.height = cell * ROWS;
nextEl.width   = cell * 4;
nextEl.height  = cell * 4;
...
const collides = (shape, ox, oy) =>
  shape.some((row, r) => row.some((v, c) => {
    if (!v) return false;
...
function lock() {
  ...
  if (cleared) {
    ...
    dropInt = Math.max(80, 800 - (level-1)*72);
...
function init() {
  clearSave();
  board = emptyBoard(); score=0; level=1; lines=0; dropInt=800;
...
/* ── DRAW ── */
function draw() {
  bCtx.fillStyle = '#060c10';
  bCtx.fillRect(0, 0, boardEl.width, boardEl.height);
  if (!board) return;
  ...
  board.forEach((row, r) => row.forEach((col, c) => {
    ...
    bCtx.fillRect(c*cell+1, r*cell+1, cell-2, 2);
    bCtx.fillRect(c*cell+1, r*cell+1, 2, cell-2);
  }));
  ...
  piece.shape.forEach((row,r) => row.forEach((v,c) => {
    if (v) { bCtx.fillStyle=piece.color; bCtx.fillRect((piece.x+c)*cell+1,(gy+r)*cell+1,cell-2,cell-2); }
  }));
  bCtx.globalAlpha = 1;

  // Active piece with glow
  bCtx.save();
  bCtx.shadowColor = piece.color;
  bCtx.shadowBlur  = 8;
  piece.shape.forEach((row,r) => row.forEach((v,c) => {
    if (v) {
      bCtx.fillStyle = piece.color;
      bCtx.fillRect((piece.x+c)*cell+1, (piece.y+r)*cell+1, cell-2, cell-2);
      bCtx.fillStyle = 'rgba(255,255,255,.22)';
      bCtx.fillRect((piece.x+c)*cell+1, (piece.y+r)*cell+1, cell-2, 2);
    }
  }));
  bCtx.restore();
  drawNext();
...
function endGame() {
  ...
  olMsg.textContent   = `⭐ ${score.toLocaleString()} · 🏆 Best: ${best.toLocaleString()}`;
...
/* ── BOOT ── */
draw(); // show empty board behind overlay

const existingSave = loadSave();
if (existingSave && existingSave.board) {
  olTitle.textContent  = '🧩 Continue?';
  olMsg.textContent    = `⭐ ${(+existingSave.score).toLocaleString()} · 🚀 Level ${existingSave.level}`;
  btnStart.textContent = '▶ Resume';
  btnStart.onclick     = () => resume(existingSave);

  const btnNew2 = document.createElement('button');
  btnNew2.className  = 'btn-play';
  btnNew2.style.cssText = 'background:rgba(255,255,255,.09);box-shadow:none;margin-top:6px;';
  btnNew2.textContent = '🔄 New Game';
  btnNew2.onclick = () => { clearSave(); overlay.style.display='none'; init(); };
